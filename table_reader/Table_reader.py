from table_reader_interface import Table_reader_interface
import cv2
import numpy as np
from scipy.stats import mode


class Table_reader(Table_reader_interface):
    ...
    def __remove_unusually_cells(self, columns_list: list,
                                 coord_num: int = 2) -> list:
        if not (2 <= coord_num <= 3):
            raise ValueError('coord_num must be equal 2 or 3')
        new_list = []

        for col in columns_list:
            m = np.median([bbox[coord_num] for bbox in col])
            new_col = []
            for bbox in col:
                param = bbox[coord_num]
                if m - 10 <= param <= m + 10:
                    new_col.append(bbox)
    ...
    def get_table_grid(self, image: np.array) -> tuple:
        ...
        boxes = self.__get_boxes(bin_im, it=5)

        contours, _ = cv2.findContours(boxes, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)

        bbox_list, cntr = self.__get_cells(contours)
        return bbox_list, cntr

    def extract_cells(self, img: np.array) -> list:
        bbox_list, cntr = self.get_table_grid(img)

        columns_list = self.__get_columns(bbox_list)  # tuples (x, y, w, h) -- cells properties
        ...
        columns_list = self.__remove_unusually_cells(columns_list, coord_num=2)
        columns_list = self.__complete_table(columns_list)
        columns_list = self.__remove_unusually_cells(columns_list, coord_num=3)
        columns_list = self.__complete_table(columns_list)
        return columns_list
